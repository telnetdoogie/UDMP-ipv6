...
* Add the two files `41-enable-he-ipv6.sh` and `99-add-cronjobs.sh` to `/mnt/data/on_boot.d` 
  * (both of these files should be executable with `chmod +x {filename}`
* Add a folder, `/mnt/data/cronjobs` and put the file `update_ipv6_chains` in that folder.
* Add a folder, `/mnt/data/ipv6` and drop both files `enable-he-ipv6.sh` and `configure-he-ipv6-chains.sh` in that folder.
* Edit the file `/mnt/data/ipv6/enable-he-ipv6.sh` and change the two properties `REMOTE_ENDPOINT` and `LOCAL_IPV6` to match the values from `Server IPv4 Address` and `Client IPv6 Address` respectively
